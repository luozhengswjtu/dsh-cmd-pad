...
- **基本操作**：分组条筛选（全部 / 项目：当前工作区 / 常驻 / ▸ 更多）→ 点击命令块或「复制」一键复制
  ...
  = 分组条下方「添加命令」长条按钮（标题/命令/备注/危险勾选/分组多选，可当场新建分组快捷路径）；
...
- **数据与手改**：命令库在 `%USERPROFILE%\.dsh\profiles\web\cmd-pad\commands.yml`（人可手改，
  ...
  命令删光分组自动消失）；可用「添加命令」长条按钮、「＋」新建分组或手改 yml 建立自己的命令库。
...
「添加命令」长条按钮 / 「＋」新建分组（调整记录 #33：入口解耦）+ 右键菜单，命令库写操作全部走 PUT `/api/library`（原子写 + `.bak`）：
