...
- **基本操作**：分组条筛选（全部 / 项目：当前工作区 / 常驻 / ▸ 更多）→ 点击命令块或「复制」一键复制
  ...
  **添加分组** = 分组栏右侧「＋」（新建分组自动设为常驻，空分组也显示；**取消常驻后保留在「更多」区不消失**，调整记录 #38）；右键菜单：卡片 = 复制/编辑/删除，
...
「添加命令」分组栏下方长条横条（#48：「添加分组」分组栏右侧 ＋，入口解耦）+ 右键菜单，命令库写操作全部走 PUT `/api/library`（原子写 + `.bak`）：

- **添加（#48 内联卡片，不再弹窗）**：点「添加命令」横条 → 当前分组命令列表顶部出现
  **未编辑卡片**：标题（必填）/命令行（必填，等宽代码块样式）/备注（可选）；底部一行 =
  备注 + 危险勾选 + ✓/✕（位于备注右侧）；**✓ 或点击卡片外 = 完成创建，✕/Esc = 取消**；
  **放弃分组选择**——新命令只归属当前选中分组（group:xxx → 该组；「项目：cwd」→ cwd；
  「未分组」→ 无分组）；**「全部」「上次使用」视图不显示添加横条**（不可添加）；
  危险关键词自动勾选保留（可取消）；
- **编辑**（表单弹窗）：标题/命令（多行等宽）/备注/危险勾选/分组多选；分组列表
...
- **回归**：`node test/t04-write-ops.test.mjs`（29 项：纯逻辑 7 + DOM 交互 22，
  含 22 次连续增删改后 yml 始终合法、＋ 新建分组全链路、内联添加卡片全链路）。
...
## 回归汇总（T08 收尾 138 项；#28 后 150 项；#33 后 152 项；#38 后 153 项；#47 后 154 项；#48 后 155 项；#49 后 156 项）

全套验收 harness **156 项全过**（`node test/<t0X>-*.test.mjs`，工作目录 `dsh-cmd-pad/`）：
...
| `t03-browse-copy` | 39 | 分组模型/消歧/搜索/**上次使用视图（记录/范围切换/ⓘ）** + DOM 渲染 + 视觉规范 §6 静态检查（含 **Toast 显示态可点击** #49） |
| `t04-write-ops` | 29 | 增删改/**内联添加卡片（当前分组归属/必填校验/外部点击提交/✕ 取消）**/删除语境语义/撤销/重命名级联/常驻持久化/新建分组（＋ 独立入口） |
