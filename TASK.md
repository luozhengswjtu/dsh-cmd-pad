...
| 2026-08-2x | 主形态（T06 后打磨） | #29 **用户反馈：搜索框末尾距「添加」按钮太近**（截图：搜索行右端 = 搜索框 + ✕ 清空 + `+ 添加` 挤在一起，间距仅容器 flex `gap:6px`）。要求设置合适的距离 | 新增作用域规则 `.cmd-pad-search .cmd-pad-add{margin-left:10px}`（叠加 flex gap 后 ✕ 与 `+ 添加` 间距 = **16px**，与搜索行左右 `padding:10px` 同量级、视觉上形成「搜索控件组 | 操作按钮」分区）；仅作用主形态搜索行（抽屉顶栏的 `.cmd-pad-add` 不在 `.cmd-pad-search` 内，不受影响）；**全套 150 项回归全过**（t02-data-layer 33 + t02-cluster-offset 7 + t03-drawer-layout 12 + t03-browse-copy 38 + t04-write-ops 25 + t06-main-form 14 + t07-run 21）+ `node --check` 全过；文档同步：视觉规范 §3.3（主形态 add 按钮间距注记）；待用户实机复核间距手感 |
| 2026-08-2x | 主形态（T06 后打磨） | #30 **用户反馈三项**（「这个搜索命令的框不够显眼」）：① 搜索框加一层**小小的阴影，不要重**；② 「这个距离可以再远一些」（承接 #29 的 16px 间距）；③ 「同样给命令内容的框也加一层」同款阴影 | ① `.cmd-pad-search-input` 加中性黑浅投影 `box-shadow:0 1px 3px rgba(0,0,0,.15)`（浮起感、不承载主题色语义，符合 t03 C1 静态检查「rgba 仅允许中性黑 rgba(0,0,0,.x)」）；② `.cmd-pad-search .cmd-pad-add` 的 `margin-left` 10px→**16px**（✕ 与「+ 添加」总间距 6+16=**22px**，视觉上形成更明确的操作按钮分区）；③ `.cmd-pad-card`（命令内容卡片，主形态/抽屉同一份代码）加同款 `box-shadow:0 1px 3px rgba(0,0,0,.15)`；**全套 150 项回归全过**（含 t03 C1 中性 rgba 检查）+ `node --check` 全过；文档同步：视觉规范 §3.3（间距 22px + 搜索框/卡片投影注记）；待用户实机复核阴影轻重与间距手感 |
| 2026-08-2x | 主形态/降级（T06 后打磨） | #31 **用户反馈：「×号应该在搜索框里面的右侧才对」**——清空 ✕ 此前是输入框外的 flex 兄弟元素（搜索行内 `[🔍][输入框][✕][+ 添加]`），用户要求 ✕ 悬在**输入框内部右侧**（常见搜索框形态） | 重构搜索行 DOM：新增 `.cmd-pad-search-field`（`flex:1; min-width:0; position:relative; display:flex; align-items:center`）包裹 input 与 ✕（field 为 ✕ 的定位上下文）；✕ 改 `position:absolute; right:4px; top:50%; transform:translateY(-50%)` 垂直居中于输入框内；输入框 `padding-right:8px→28px` 为 ✕ 留位（文字不重叠）；`+ 添加` 仍是 `.cmd-pad-search` 直接子元素（t06 N1 断言不破），计数 `命中 N 条` 留在 field 与添加按钮之间；两形态共用同一份 `createPanelBody` 自动一致；**全套 150 项回归全过**（t03 C7 结构 / t06 N1 addBtn 直子 / B9 计数 / B10 Esc 清空 / N6 Esc 链全过）+ `node --check` 全过；文档同步：视觉规范 §3.3（搜索框结构注记）；待用户实机复核 |
| 2026-08-2x | 主形态/降级（T06 后打磨） | #32 **用户反馈：命令内容框内的「写命令行内容」的块也加一层阴影，且比命令内容框的阴影淡一点**——即卡片内的命令行代码块 `.cmd-pad-card-cmd`（等宽字体、bg-layer-1 + border-l1）加投影，强度低于卡片本体 | `.cmd-pad-card-cmd` 加**更淡的中性黑浅投影** `box-shadow:0 1px 3px rgba(0,0,0,.08)`（卡片本体为 `.15`，代码块减半强度，同几何 1px/3px，浮起层级：卡片 > 代码块 > 底）；两形态共用同一份 CSS 自动一致；**全套 150 项回归全过**（含 t03 C1 中性 rgba 静态检查）+ `node --check` 全过；文档同步：视觉规范 §3.3（投影注记补代码块）；待用户实机复核 |
| 2026-08-2x | 主形态/降级（T06 后打磨） | #33 **「添加命令」与「添加分组」解耦（用户定稿方案）**。讨论后用户给出明确设计：① **添加分组按钮放在分组栏的右侧，只显示一个加号**；② **添加命令改为分组栏下方一个长条按钮，写着「添加命令」**；③ 表单「新建分组名」行**保留为快捷路径**；④ 本阶段**只做入口解耦**（空组自动常驻，不动数据模型/分组聚合） | 已实现：`createPanelBody` 新增 **`.cmd-pad-addcmd`**（分组条下方全宽长条按钮「添加命令」，`margin:6px 10px 0` + border-l1 透明底）与 **`.cmd-pad-group-add`**（分组栏右侧「＋」纯加号 chip，`margin-left:auto` 靠右）；**旧 `.cmd-pad-add` 全部移除**（搜索行右端 + 抽屉顶栏，对应 CSS 与 `opts.addBtn` 机制删除）；「＋」→ `buildAddGroupModal`（输入分组名，回车可提交）→ `submitGroup`（校验：非空/无换行/非路径名/不重名 → **自动置入 state.pinnedGroups 常驻**（空组也显示，count=0）→ persistState → Toast「已创建分组」）；两形态共用骨架自动一致。**测试**：t03 C7 更新（body 子元素 3→4：search/groups/addcmd/content）、t04 E1 重写 + 新增 **E5b**（＋ 建组全链路：弹窗/自动常驻/state 持久化/分组条出现空组/Toast）与 **E5c**（空名/重名/路径名拒绝 + 回车创建 + Esc 关闭）、t06 N1/N6 选择器更新——**全套 152 项回归全过**（t02 33 + t02-offset 7 + t03-drawer 12 + t03-browse 38 + t04 27 + t06 14 + t07 21）+ `node --check` 全过；文档同步：功能文档 §3.5/§7 F6、视觉规范 §3.3（入口配方 + 旧按钮移除）、README（基本操作/写操作/回归汇总 152 项）；**待用户实机复核**：＋ 在分组栏右侧的视觉效果（分组多时换行后的落位）、长条按钮样式 |
| 2026-08-2x | 主形态/降级（T06 后打磨） | #34 **用户反馈三项**：① 「搜索框和命令框，以及命令行代码块的阴影都再淡一点」；② 「搜索框左侧的搜索图标直接去掉」；③ 「添加命令改为放在搜索框右侧，且和搜索框的右侧末尾保持一定距离」 | ① 三处投影再淡：`.cmd-pad-search-input` `.15→.10`、`.cmd-pad-card` `.15→.10`、`.cmd-pad-card-cmd` `.08→.05`（浮起层级不变：卡片 > 代码块 > 底，均中性黑 rgba(0,0,0,.x) 过 t03 C1）；② **移除搜索框左侧放大镜**（SEARCH_SVG 常量、`searchIcon` span、`.cmd-pad-search svg` CSS 规则全部删除——内联 SVG 3 处→**2 处**（FAB + ⓘ），t03 C4 断言同步，AGENTS.md 规则 7 / 视觉规范 §3.1/§3.2/§6 同步「2 处」；搜索语义由占位文案「搜索命令（/）」承载）；③ **「添加命令」从分组条下方长条改回搜索框右侧**：`.cmd-pad-addcmd` 移入 `.cmd-pad-search`（field/count 之后），样式改紧凑按钮（`flex:none` + `margin-left:14px`，叠加 6px gap = 与搜索框右侧末尾总间距 **20px**，满足「保持一定距离」）；`＋` 新建分组仍在分组栏右侧（#33 保留）。**测试**：t03 C4（SVG 3→2）+ C7（body 子元素 4→3 恢复 search/groups/content）、t04 E1（addcmd 位于搜索行 + 搜索行无 svg）、t06 N1 同步——**全套 152 项回归全过** + `node --check` 全过；文档同步：功能文档 §3.5/§7 F6、视觉规范 §3.1/§3.2/§3.3/§6、README（基本操作/写操作）、AGENTS.md 规则 7；**待用户实机复核**：阴影淡度、搜索框无图标后的观感、添加命令与搜索框的间距 |
| 2026-08-2x | 主形态/降级（T06 后打磨） | #35 **用户反馈三项**：① 「添加命令也加同样的阴影」；② 「拉长一点这个按钮」（添加命令）；③ 「分组栏右侧的加号再大号一点」 | ① `.cmd-pad-addcmd` 加**与搜索框/卡片同款投影** `box-shadow:0 1px 3px rgba(0,0,0,.10)`，底色改 `transparent→bg-base`（有投影后与搜索框观感一致，浮起感成组）；② 拉长：`padding:6px 10px→6px 18px`（左右各 +8px，按钮约宽 1.3 倍）；③ ＋ 放大：`min-width:22px→26px; height:20px→24px; font-size:13px→15px`（与「更多」箭头同字号）；`margin-left:14px` 与分组栏右侧布局不变；**全套 152 项回归全过** + `node --check` 全过；文档同步：视觉规范 §3.3（addcmd 投影/尺寸 + ＋ 尺寸）；待用户实机复核 |
| 2026-08-2x | 主形态/降级（T06 后打磨） | #36 **用户反馈三项**：① 「这个加号里面的大小感觉没变，调大一点」；② 「添加命令的边框倒角和其他不一致」；③ 「运行和复制按钮，保持和命令行代码块一样的阴影」 | ① ＋ 换**全角加号 U+FF0B** + `font-size:15→16px` + 盒子 `26×24→28×26`（根因：ASCII `+` 在多数字体里纤细，只加大字号不显眼——全角加号才真正变大）；② **添加命令与「运行/复制」、命令行代码块统一为同套小控件配方**：底色 `bg-base→bg-layer-1`、投影 `.10→.05`、圆角统一 **6px**（#36 说明：addcmd/复制/运行/代码块原已全为 6px，观感不一致来自底色与阴影层级不同，统一配方后一致）；③ `.cmd-pad-btn`（运行/复制）加**与代码块同款投影** `box-shadow:0 1px 3px rgba(0,0,0,.05)`；搜索框/卡片保持 `.10`（输入框/内容层，与小按钮不同层级）。**测试**：t04 E1 断言 `+`→`\uFF0B`——**全套 152 项回归全过** + `node --check` 全过；文档同步：视觉规范 §3.3（addcmd 配方统一 + ＋ 全角 + btn 投影）；待用户实机复核 |
| 2026-08-2x | 主形态/降级（T06 后打磨） | #37 **用户定位「添加命令」按钮违和感根因：「和搜索框的高度不一致」，要求保持一致** | 显式等高：`.cmd-pad-search-input` 与 `.cmd-pad-addcmd` 均加 `height:26px` + `box-sizing:border-box`（输入框原内容驱动约 24px 且无边框、按钮约 26px 且带 1px 边框，高度差是违和感来源）；按钮垂直内边距 `6px→5px`（高度由 height 固定）；内部 ✕ 由 `top:50%` 居中不受影响；搜索行 `align-items:center` 下两者顶部/底部严格对齐。**测试**：无结构变化，**全套 152 项回归全过** + `node --check` 全过；文档同步：视觉规范 §3.3（addcmd 与搜索框等高注记）；待用户实机复核 |
| 2026-08-2x | 主形态/降级（T06 后打磨） | #38 **用户实测反馈 bug：「＋」新建的「常用」分组默认常驻，取消常驻后没有看到「更多」图标出现**。根因：空分组**只存在于 `state.pinnedGroups`**（不参与命令聚合），取消常驻即从模型中彻底消失——`unpinnedCustom` 只统计聚合分组，「更多」计数不含它 | 修复：新增 **`state.customGroups`（显式分组名单）**——① `＋` 新建分组同时记入 `customGroups`（`submitGroup`）；② **取消常驻时空分组记入 `customGroups` 保留**（`toggleGroupPinned`，非空分组记入也无害，模型做聚合并集去重）；③ `buildGroupModel` 的 `unpinnedCustom` 改为「聚合 ∪ customGroups」并集 − 常驻 → 空组显示在「更多」区、「更多」箭头出现；④ `isValidView` 增加 `unpinnedCustom` 判定（取消常驻后视图仍有效）；⑤ `allGroupNames` 纳入 customGroups（重名检测覆盖）；⑥ 分组**删除**同步清理 `pinnedGroups`/`customGroups`、**重命名**同步更新两者（不残留/不陈旧）。兼容：SAMPLE_STATE 无 customGroups → 默认 `[]`，模型行为不变。**测试**：t04 新增 **E5d**（＋ 建空组 → 取消常驻 →「更多」箭头出现 + 展开可见 + customGroups 持久化 + 视图有效空态）——**全套 153 项回归全过**（t02 33 + t02-offset 7 + t03-drawer 12 + t03-browse 38 + t04 28 + t06 14 + t07 21）+ `node --check` 全过；文档同步：功能文档 §3.5（空分组不消失语义）、README（基本操作）；待用户实机复核 |
| 2026-08-2x | 全部（发布准备） | #39 **发布前核对（用户确认「其他窗口完成细节修复，准备发布」）**。核对发现 3 处发布问题：① `package.json` `version` 仍为 `0.1.0`（实际产品已 v0.2：双形态 + 运行通道 + 上次使用视图）；② `description` 仍写「运行功能暂缓，见调整记录 #21」（#21 是旧方案被否，运行已按 #24 于 T07 落地）——过时描述会误导 npm/生态展示；③ README「回归汇总」停留在 #33 的 **152 项**（#38 后实际 **153 项**，t04 27→28），且安装命令「<本目录>」不够明确。同时**实证安装链路**：`node_modules/dsh-cmd-pad` junction → 插件目录 + profile `package.json` `dependencies` 含 `"dsh-cmd-pad":"link:…"` + `dsh.profile.bundles` 含 `dsh-cmd-pad`——`dsh plugin --profile web add <插件包目录>` 可正常安装 | 已修复：① `package.json` `version` → **`0.2.0`**；② `description` 重写（双形态 + 一键运行 + 上次使用视图 + 危险双确认，去掉「暂缓」）；③ README 回归汇总 **152→153 项**（t04 28 项，章节 + 表格 + 标题行三处）+ 安装说明明确化（`<插件包目录>` = 含 package.json 的目录，补「安装后入口检查」：主形态侧边卡片/+ 菜单、降级浮动图标）。**153 项回归全过**（t02 33 + t02-offset 7 + t03-drawer 12 + t03-browse 38 + t04 28 + t06 14 + t07 21）+ package.json JSON 校验 + `node --check` 全过；**v0.2.0 tag 仍未打**（用户此前表示有「其他建议」待补充；发布锚点如需随时可补） |
| 2026-08-2x | 全部（发布收尾） | #40 **v0.2.0 tag 已打 + 本机数据彻底清理（用户定夺）**。用户确认：① 打 v0.2.0 tag；② 删除 commands.yml.bak（6 条演示命令备份，含危险命令）；③ 顺带发现并清除其他会话真机测试残留（commands.yml 2 条测试命令「测试/测试2」、state.json 的 recentCommands/customGroups「常用」/viewLastUsedAt） | ① `git tag v0.2.0` → 指向 `64879aa`（发布前核对提交）；② `.bak` 删除（含清库时插件自动生成的备份，最终数据目录 = `commands.yml`(空库 13B) + `state.json`(4B)，**真正发布全新状态**，API 实证 `{"library":{"commands":[]},"state":{}}`）；③ 测试残留经 PUT 清空 + state.json 原子重置 `{}` 清除。注意：state.json 重置与 .bak 删除均绕过 API（PUT /api/state 为增量合并无法删键；.bak 由原子写自动生成），走文件级操作，需对 profile 目录写权限（本会话获批 danger-full-access）。**发布就绪**：代码 153 项回归全过、package.json v0.2.0、README 完备、安装链路实证、tag 已打、本机全新 |
| 2026-08-2x | 全部（发布完成） | #41 **推送 GitHub（用户确认远端仓库已创建）**。用户反馈「git 仓库看是空的」——核查：本地 44 个提交 + v0.2.0 tag 完好，但**从未 push**（账本多处「本地未推送」），远端 `luozhengswjtu/dsh-cmd-pad` 空是预期。排障链：① 网络 `Connection was reset` → 本机系统代理 `127.0.0.1:7897` 开着但 git 未配 → 仓库级 `http.proxy`/`https.proxy` 指向代理后连通；② 认证 `could not read Username`（credential.helper=manager 但 GCM 未装；gh 已登录 luozhengswjtu 含 repo scope）→ 仓库级 `credential.helper='!gh auth git-credential'` 因沙箱 sh 崩溃失效 → 改用 **GIT_ASKPASS 临时脚本输出 `gh auth token` + 仓库级 `credential.username`** 成功（git 直接执行 .cmd 不走 sh） | **推送完成**：`git push -u origin main`（44 提交，`refs/heads/main`→`8d40457`）+ `git push origin v0.2.0`（`refs/tags/v0.2.0`→`64879aa`），ls-remote 实证。**发布闭环**：GitHub `luozhengswjtu/dsh-cmd-pad` 可 clone 安装（`git clone` → `dsh plugin --profile web add dsh-cmd-pad`）。经验：git 走代理与凭据（gh CLI 登录即可 push，无需 GCM；GIT_ASKPASS 脚本法适配受限环境）；仓库级配置保留 `http.proxy`/`credential.username`（无害），临时 askpass 已清理 |
