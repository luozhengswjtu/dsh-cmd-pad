...
| 2026-08-2x | 主形态/降级（T06 后打磨，设计讨论后定稿） | #33 **「添加命令」与「添加分组」解耦（用户定稿方案）**。讨论后用户给出明确设计：① **添加分组按钮放在分组栏的右侧，只显示一个加号**；② **添加命令改为分组栏下方一个长条按钮，写着「添加命令」**；③ 表单「新建分组名」行**保留为快捷路径**；④ 本阶段**只做入口解耦**（空组自动常驻，不动数据模型/分组聚合） | 已实现：`createPanelBody` 新增 **`.cmd-pad-addcmd`**（分组条下方全宽长条按钮「添加命令」，`margin:6px 10px 0` + border-l1 透明底）与 **`.cmd-pad-group-add`**（分组栏右侧「＋」纯加号 chip，`margin-left:auto` 靠右）；**旧 `.cmd-pad-add` 全部移除**（搜索行右端 + 抽屉顶栏，对应 CSS 与 `opts.addBtn` 机制删除）；「＋」→ `buildAddGroupModal`（输入分组名，回车可提交）→ `submitGroup`（校验：非空/无换行/非路径名/不重名 → **自动置入 state.pinnedGroups 常驻**（空组也显示，count=0）→ persistState → Toast「已创建分组」）；两形态共用骨架自动一致。**测试**：t03 C7 更新（body 子元素 3→4：search/groups/addcmd/content）、t04 E1 重写 + 新增 **E5b**（＋ 建组全链路：弹窗/自动常驻/state 持久化/分组条出现空组/Toast）与 **E5c**（空名/重名/路径名拒绝 + 回车创建 + Esc 关闭）、t06 N1/N6 选择器更新——**全套 152 项回归全过**（t02 33 + t02-offset 7 + t03-drawer 12 + t03-browse 38 + t04 27 + t06 14 + t07 21）+ `node --check` 全过；文档同步：功能文档 §3.5/§7 F6、视觉规范 §3.3（入口配方 + 旧按钮移除）、README（基本操作/写操作/回归汇总 152 项）；**待用户实机复核**：＋ 在分组栏右侧的视觉效果（分组多时换行后的落位）、长条按钮样式 |
